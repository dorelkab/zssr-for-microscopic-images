...
class ZSSR:
    ...
    def father_to_son(self, hr_father):
        ...
        # plt.imshow(hr_father, cmap='gray')
        # plt.show()
        # sys.stdout = open(os.devnull, 'w')
        # raw_data = RawData.from_arrays(X= [hr_father],Y= [hr_father],axes= 'CYX')
        ...
        #         raw_data=raw_data,
        #         patch_size=(hr_father.shape[0], hr_father.shape[1],hr_father.shape[2]),
        ...
        #     # plt.imshow(hr_father, cmap='gray')
        #     # plt.show()
        #     d = (int(hr_father.shape[0]/2), int(hr_father.shape[1]/2))
        #     lr_son = cv2.resize(hr_father, d)
        # else:
        #     d = (int(hr_father.shape[0]/2), int(hr_father.shape[1]/2))
        #     lr_son = cv2.resize(Y[0], d)
        #     # plt.imshow(lr_son, cmap='gray')
        #     # plt.show()
